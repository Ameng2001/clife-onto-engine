...
`act` 提交成功后,系统 SHALL 把 `ActionResult.written` 对应的**已提交**对象从引擎 store 读出,经 UModel REST `entities:write` 反映进读层 workspace(成形复用导出器 `_eid`/payload)。`rejected` 与 `pending_hil` MUST NOT 反映。
...
- **THEN** 读层不发生任何写入,UModel 对象图不变

#### Scenario: HIL 待审不反映
- **WHEN** Action 命中 HIL 关口,引擎返回 pending_hil(未提交)
- **THEN** 读层不反映;`act` 原样返回 pending_hil 供人工复核
