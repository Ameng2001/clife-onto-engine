...
# 避免安全字段因**键名漂移绕过「霉变拦截」**——真 Qwen record-replay 抓到过 mycotoxin 绕过。
_MEASURE_ALIAS = {"mycotoxin": "霉菌毒素", "aflatoxin": "霉菌毒素",
                  "黄曲霉毒素": "霉菌毒素", "mold_toxin": "霉菌毒素"}


def _normalize_measurements(m):
    return {_MEASURE_ALIAS.get(k, k): v for k, v in (m or {}).items()}
...
def quick_test_grading(ctx) -> None:
    ...
    m = _normalize_measurements(ctx.params["measurements"])   # 键归一：安全字段不因 LLM 命名漂移而漏拦
