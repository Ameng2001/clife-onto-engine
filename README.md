...
- [x] **附着知识（多场景知识支持：吸收 Palantir 统一图 + UModel 模板化）**：给业务对象声明**附着知识** `KnowledgeItem`（标准化 kind：分析模板/诊断/处置手册 playbook/领域参考 + 内容 + 出处），与对象**一次取到**（`knowledge_for`，Palantir 式同图不跨层），Explorer 点对象即见（属性+知识+遥测一屏）；与强制知识（Rule）、派生知识（Function）**并存**——一套引擎覆盖强制/派生/参考/诊断多种知识性质；声明式（槽位2，镜像遥测绑定）、行业无关
