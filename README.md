...
>
> **展示/遥测可借 [UModel](https://github.com/alibaba/UnifiedModel) 互操作上车**：把本体导成 UModel model pack（`scripts/export_umodel.py`），即白捡 Web Explorer / SPL / MCP 读工具。注意分层——**语义读（OQL 查受治理对象图）是引擎自有**、是 OS 脊椎；UModel 提供的是**展示皮 + 遥测管道**这类成熟件，定位是**互操作 / 上车坡道**（非永久读层），路线图逐步内化。定位/契约/内化路线见 [`docs/04-umodel-interop.md`](docs/04-umodel-interop.md)。
>
> 建模端的同款分层视图（一份分析→两层）见 astra-studio 的 [**Full Workflow**](https://github.com/Ameng2001/astra-studio-plugins#full-workflow)。
...
python -m pytest tests/ -q             # 28 项：引擎/沙箱/OQL/记忆/编排/原子性/持久化/OKF
```
...
python scripts/export_explorer.py             # 运行时对象图 → 自有 Explorer（离线单文件 HTML，零依赖 UModel）
python scripts/export_umodel.py               # 本体 → UModel model pack（互操作：Explorer/SPL/MCP）
python scripts/smoke_umodel.py                # UModel pack 离线 schema 校验（对 vendored 规格）
python scripts/smoke_mcp_bridge.py            # 治理写桥：写只经引擎、本体兜底、只反映已提交、读层解耦
python scripts/smoke_telemetry.py            # 遥测 query-plan（自有深读·遥测侧）：对象→PromQL 计划、id 代入、防注入、不触网
python scripts/e2e_harness.py --stub         # 端到端 harness（桩）：整栈跑一段会话 → 查/提交/治理拒绝/澄清 + 审计
# 切真 Qwen：放好 llm.local.json（或设 DASHSCOPE_*）后去掉 --stub，即用真意图编译器压整栈（看 LLM 选没选对、治理有没有兜住）
python scripts/e2e_knowledge_influence.py --stub  # 知识影响力对比：同句口语 load_knowledge 关/开各跑，SpyCompiler 验知识到达 LLM；真 Qwen 下看知识改没改变 LLM 选择
python scripts/e2e_corpus.py --stub            # 端到端场景语料：读/写全面覆盖矩阵（两插件 4 动作各提交+拒绝、简单/多跳/聚合读、advise、clarify）；去 --stub 走真 Qwen 抽检意图编译准确率
```

> UModel 互操作与治理写桥的定位/契约/红线/内化路线见 [`docs/04-umodel-interop.md`](docs/04-umodel-interop.md)；
> 真实 UModel 在线验收（需 docker 起 sidecar）：`scripts/verify_umodel_online.sh`、`scripts/verify_mcp_bridge_online.sh`；
> 接 MCP agent：`scripts/serve_mcp.py` + [`.mcp.json.example`](.mcp.json.example)。
...
**HTTP 服务（把 Session 包成 API，FastAPI）：**

```bash
pip install -r requirements-server.txt
python scripts/serve.py                       # http://127.0.0.1:8000 （/docs 看 OpenAPI 接口）

curl -s localhost:8000/manifest/grass         # 能力清单（能做什么/能查什么）
curl -s -XPOST localhost:8000/ask -H 'content-type: application/json' \
     -d '{"ontology":"grass","utterance":"巴彦淖尔有哪些地块？"}'
# → {"kind":"query","rows":[{"parcel_id":"parcel_001",...}], "query":{"start":"Site",...}}
curl -s -XPOST localhost:8000/ask -H 'content-type: application/json' \
     -d '{"ontology":"grass","utterance":"给 parcel_001 出方案，用紫花苜蓿，预算300"}'
# → {"kind":"rejected","violations":[{"rule":"乡土合规",...}]}   # LLM 提议、本体兜底
```

端点：`POST /ask`（做/查/澄清）· `GET /manifest/{ontology}` · `GET /audit/{ontology}` ·
`GET /ontologies` · `GET /health`。同一服务托管多个本体（grass/chili），按 namespace 隔离。
...
LLM 凭据只从**环境变量**注入（`OpenAICompatibleClient` 先读 env）；`.dockerignore` 排除
`llm.local.json`/`raw-input`/`reference` —— **镜像里零密钥、零商密**。自带 `HEALTHCHECK`。
...
| [`docs/04-umodel-interop.md`](docs/04-umodel-interop.md) | UModel 互操作：语义读(OQL)自有 vs 借 UModel 展示/遥测；契约映射 + 红线 + 内化路线（Palantir-true） |
| [`docs/05-e2e-validation.md`](docs/05-e2e-validation.md) | 端到端验证报告（真 Qwen 实证）：整栈回路 + 治理兜底 + 知识防错 + 读写全覆盖 + 对抗鲁棒性的真 LLM 结果与结论 |
| [`docs/06-deployment.md`](docs/06-deployment.md) | 部署 Runbook：认证→租户边界→授权门四层 + 声明式租户数据接入 + 启动/冒烟/延迟成本，从零到「真实用户能打一次」 |
...
- [x] **UModel 互操作（导出 + 治理写桥）**：导出器（五要素→UModel pack，与 `okf.py` 同层）让治理对象图进 Web Explorer / SPL / MCP；MCP 写桥 `act` 经引擎（guard→回滚→审计）、仅 committed 反映（含关系），**不开 UModel 写旁路**；grass + chili 双本体均用真实 UModel 验证。定位为**互操作/上车坡道**（语义读 OQL 自有，路线图内化遥测+展示）—— 见 [`docs/04-umodel-interop.md`](docs/04-umodel-interop.md)
- [x] **本体版本化 + 决策重放（治理化变更生命周期地基）**：`versioning.py` 把某本体某时刻的五要素快照成不可变、可寻址、可执行的版本（多版本共存、互不污染）；`replay.py` 用审计快照的 inputs 只读重跑 `validate` —— 复现裁决、**反事实**（换入参看翻转）、**版本敏感**（对更严规则版本重放翻转）。是规则变更影响分析 / CQ 验收的共享原语（边界：function-backed 规则读调用方 store，点位历史 store 留扩展）
- [x] **规则变更影响分析（B2）**：`change_impact.py` 对一批历史决策，同 store/同 inputs 分别对旧/新本体版本重放并 diff，产出**变更影响报告**（新拦/新放/不变/跳过 + 每条翻转的**触发规则**）——同 store 隔离规则改动本身的影响，上线前可评估「改这条规则会翻转哪些历史决策」
- [x] **CQ 验收回路（C3）+ 治理化变更生命周期闭环**：`cq.py` 声明式 competency questions（动作期望裁决 + 查询期望结果）对某本体版本跑 pass/fail，`plugins/grass` 带一组 CQ（槽位7）；同套 CQ 对不同版本跑 = **能力回归门**（版本削弱规则即被抓）。至此 **本体版本化 + 决策重放 + 变更影响(B2) + CQ 验收(C3)** 合成「活的本体·治理化变更生命周期」闭环：建模端改本体产出新版本 → 变更影响 + CQ 验收 → 回喂建模端
- [x] **本体治理缺口审计（C1 运行时侧）**：`gaps.py` 静态扫 registry 报缺口——**blocking**（无 handler 的 Action / 悬空规则·写·关系端点引用 / 无 impl 的 Function，会运行时炸）+ **advisory**（规则无出处，补 OKF）；补 studio-ontology 骨架的 TODO(FDE) 缺口、上线前静态可查（codegen 回填仍在建模端）
...
- [x] **pytest 测试套件 + GitHub Actions CI**：离线确定性测试（28 项核心 + 6 项 HTTP，后者无 fastapi 时跳过）+ 内核防腐 + 插件能力检查，push/PR 自动跑（Python 3.10 / 3.12）
- [x] **HTTP API（FastAPI）**：`POST /ask`（做/查/澄清）+ manifest/audit/ontologies；同服务多本体（grass/chili）namespace 隔离；compiler 工厂注入（可 stub 离线测试）—— `scripts/serve.py`
- [x] **容器化（Dockerfile + .dockerignore）**：266MB 镜像，HEALTHCHECK，密钥运行时 env 注入不进镜像，真容器内 Qwen 端到端验证（healthy）
- [x] **记忆持久层（SqliteMemoryStore）**：四层记忆 SQLite 持久后端，与内存版 `MemoryStore` 同接口（子类 write-through 复用四层逻辑），记忆跨进程/重启不丢；同 trust SQLite 模式
- [x] **声明式授权（生产化·多租户 A 弧起步）**：`authz.py` `AuthzPolicy`（(ontology,action)→授权角色，租户可配·YAML·可 default-deny）+ 引擎**前置授权门**——调用者角色无权执行某动作，在 guard/执行**之前** `StructuredRejection(phase=authz)` + 审计 `unauthorized`（越权无副作用、留痕）；与插件业务 guard 分离（插件声明需求、租户配置授权）；默认不注入即向后兼容
- [x] **租户→本体访问边界（A 弧）**：`TenantAccessPolicy`（tenant→可访问本体，租户可配·YAML·可 default-deny）+ HTTP 服务边界强制——各本体端点（/ask·/plan·/explorer·/manifest·/audit）校验调用者租户对目标本体的访问权，跨租户/跨本体请求**入口即 403（不进引擎）**；把隔离从 Capability 内延伸到服务边界（边界+内隔离双层）；默认不设策略即向后兼容
- [x] **服务边界身份解析（A 弧·闭环访问控制）**：`identity.py` `Principal(tenant/actor/role)` + 可插拔 `IdentityResolver`（`StaticIdentityResolver` api_key→Principal，真部署注入 JWT/OIDC）；`web.py` 各端点从 `X-Api-Key` 认证 → 无效 **401**，用**认证身份**（非客户声明）驱动租户边界与授权门/动作角色。至此 **认证 → 租户边界 → 授权门** 闭环；默认不设 resolver 即向后兼容
- [x] **Capability 越界运行时审计（A 弧·安全可观测）**：Action handler 越界（沙箱拦截 stage 未声明类型/跨租户/内核直达）时，引擎**确定性回滚**（无残留写）+ **安全审计留痕**（`decision=capability_violation`，谁试图突破隔离、试了什么）+ **结构化拒绝**（`phase=capability`），而非崩溃；`validate()` 同样捕获。把安全边界从「崩」变「可观测+兜底」
- [x] **附着知识（多场景知识支持：吸收 Palantir 统一图 + UModel 模板化）**：给业务对象声明**附着知识** `KnowledgeItem`（标准化 kind：分析模板/诊断/处置手册 playbook/领域参考 + 内容 + 出处），与对象**一次取到**（`knowledge_for`，Palantir 式同图不跨层），Explorer 点对象即见（属性+知识+遥测一屏）；与强制知识（Rule）、派生知识（Function）**并存**——一套引擎覆盖强制/派生/参考/诊断多种知识性质；声明式（槽位2，镜像遥测绑定）、行业无关；**已喂进四层记忆 BACKGROUND 层**（`load_into_memory` / `Session(load_knowledge=True)`）——`ask` 装配记忆时按相关性把相关知识注入意图编译器上下文，LLM 据领域知识（诊断/处置手册/模板/参考）推理，闭合「声明→检索→呈现→**推理**」
- [x] **咨询路径 advise（知识接地的只读建议，闭合知识消费）**：意图编译器对判断类问题（该怎么做/行不行/先做什么）产出 `kind=advise`（`answer` 基于附着知识的领域建议），`Session.ask` 作为**一等只读回复**返回——**不进 Action 引擎、不写库、不越权**，进 CONTEXT 记忆；与受治理动作分层（建议低风险只读、动作受治理兜底）。补齐真 Qwen 验证暴露的缺口：LLM 能基于处置手册正确回答「重度盐碱先改良再喷播」，不再被硬塞进澄清
- [x] **端到端验证 harness**（`scripts/e2e_harness.py`）：一个脚本把整栈（口语→记忆接地+知识注入→意图编译→查 OQL/做 Action（guard→回滚→审计）→回写记忆）在**真实感数据**上端到端压一遍；**离线跑脚本编译器（确定性参照）、放好 `llm.local.json` 即一键切真 Qwen**——真 LLM 下看它选没选对动作、记忆/知识有没有帮上、治理有没有兜住。含治理兜底 money-shot（合规提交 + 非乡土/跨区域用错草种被「乡土合规」确定性拒绝）与对象图 Explorer 导出（`--explorer`）
- [x] **租户数据接入（原型硬编码 → 产品声明式，A 弧）**：`clife_onto_engine/tenant/`（行业无关加载器）从租户清单 `tenants/<t>/tenant.yaml` 把数据源（csv/jsonl/json）**按本体 schema 校验**（必填/类型强制/主键·支持组合模板）后落库，脏行**逐条拒绝留痕**（缺主键/缺必填/数字非法/未声明对象）、不静默丢、不崩，产可审计 `IngestReport`（载入/拒绝+原因/完备度）；`tenants/mengcao`（tenant-zero 问草）带清单 + **样例数据（SAMPLE）**，`scripts/tenant_load.py` 加载后在**加载的租户数据**（非 seed）上跑该本体 CQ 验收（4/4）——不改插件代码即可灌数，替代硬编码 `seed_reference_data` 的生产接缝
