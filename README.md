...
>
> **读层可由 [UModel](https://github.com/alibaba/UnifiedModel) 承担**：把本体导成 UModel model pack（`scripts/export_umodel.py`），即白捡 Web Explorer / SPL 受控读 / MCP 读工具——治理对象图能被浏览、被任何 MCP agent 读取。UModel 是引擎之上的**只读层**，受治理的写永远留在引擎。层次纪律与契约映射见 [`docs/04-umodel-interop.md`](docs/04-umodel-interop.md)。
>
...
python scripts/export_umodel.py               # 本体 → UModel model pack（读层：Explorer/SPL/MCP）
...
> UModel 读层与治理写桥的定位/契约/红线见 [`docs/04-umodel-interop.md`](docs/04-umodel-interop.md)；
...
| [`docs/04-umodel-interop.md`](docs/04-umodel-interop.md) | UModel 互操作：只读语义层（Explorer/SPL/MCP）vs 受治理写层；五要素↔UModel kinds 契约映射 + 红线 |
...
- [x] **UModel 互操作（读层 + 治理写桥）**：导出器（五要素→UModel pack，与 `okf.py` 同层）让治理对象图进 Web Explorer / SPL / MCP；MCP 写桥 `act` 经引擎（guard→回滚→审计）、仅 committed 反映进读层（含关系），**不开 UModel 写旁路**；grass + chili 双本体均用真实 UModel 验证（读+写层「换行业零改」）—— 见 [`docs/04-umodel-interop.md`](docs/04-umodel-interop.md)
