...
- [ ] commit-path 原子性补偿（非事务后端的 WAL/补偿）
- [ ] OQL 谓词下推 nGQL WHERE + 映射驱动的原生列存储
...
> 本仓库为"数智本体引擎"的工程实现与验证骨架。`third-party/okf-visualizer/` 为 vendored 第三方
> （Google，Apache-2.0，见其 `PROVENANCE.md` 与 `LICENSE.md`）。
