...
1. **内核级遥测/query-plan 能力** —— ✅ **多方言(metric/prometheus + log/elasticsearch)+ 运行时过滤 + 已对 agent 暴露**:映射层 `TelemetryBinding`(provider 在 series 级——同对象 metric 走 Prometheus、log 走 ES),`query/telemetry.py` `build_plan` 据对象实例**生成可执行计划**(PromQL/ES DSL,id 已代入、防注入),**运行时 `params`** 解析对象字段外的占位(log 的 level/时间窗等),未解析占位即拒;引擎**只产计划、不当 TSDB**。grass `Site` 已绑定墒情/NDVI(metric)+ IoT 告警日志(log/ES)。**已经 HTTP `POST /plan` 与 MCP `plan` 工具(默认开)暴露给 agent**(均透传 `params`)。**SQL 方言、派生遥测视图**留扩展点(同骨架加绑定,机制零改——已由 metric+log 双方言证明 provider 无关)。
2. **自有展示** —— ✅ **已落(离线单文件 + 活端点)**:`clife_onto_engine/explorer.py` 把运行时对象图(实例+关系)渲染成**自包含离线 HTML**(cytoscape 内联、类型上色、实例检视、图例);静态导出 `scripts/export_explorer.py`(→ `build/explorer/<ont>.html`)+ 活服务 `GET /explorer/{ontology}`(浏览实时治理状态)。**零新依赖**(复用 vendored cytoscape)、行业无关、第三方无关(JS 调用层注入)。至此浏览 UI 不再依赖 UModel Explorer。**已联动遥测**:点对象节点看它的遥测序列(name/provider/kind),活端点里 metric 可就地"取计划"(经 `/plan` 取回已代入 id 的 PromQL)——语义读(对象图)与遥测读(query-plan)在同一 UI 合体。搜索/筛选、log 序列 params 输入留扩展点。

**内化后 UModel 的角色**:降为**互操作 / import-export**(把我们的本体与外部 UModel 部署互通),不再是读/展示路径的脊椎。导出器/写桥保留为互操作通道。
