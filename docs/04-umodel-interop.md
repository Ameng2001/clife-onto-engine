...
2. **自有展示** —— ✅ **已落(离线单文件 + 活端点)**:`clife_onto_engine/explorer.py` 把运行时对象图(实例+关系)渲染成**自包含离线 HTML**(cytoscape 内联、类型上色、实例检视、图例);静态导出 `scripts/export_explorer.py`(→ `build/explorer/<ont>.html`)+ 活服务 `GET /explorer/{ontology}`(浏览实时治理状态)。**零新依赖**(复用 vendored cytoscape)、行业无关、第三方无关(JS 调用层注入)。至此浏览 UI 不再依赖 UModel Explorer。搜索/筛选、对象↔遥测联动留扩展点。
