...
1. **内核级遥测/query-plan 能力** —— ✅ **已落第一块砖(metric/prometheus)**:映射层增 `TelemetryBinding`(对象→可观测后端 + 生成器模板),`query/telemetry.py` `build_plan` 据对象实例**生成可执行 PromQL**(id 已代入、防注入),引擎**只产计划、不当 TSDB**(与 UModel 同立场)。grass `Site` 已绑定墒情/NDVI 指标(`scripts/smoke_telemetry.py`、`tests/test_telemetry_plan.py`)。**log/ES、SQL 方言、HTTP/MCP 暴露**留扩展点(同骨架加绑定即可,机制不变)。
